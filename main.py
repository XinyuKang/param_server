...
def add_args(parser):
    ...
    parser.add_argument(
        "--enable-gpu", type=bool, default=False, help="whether to use gpu"
    )
    parser.add_argument(
        "--batch-size", type=int, default=128, metavar="BS", help="training batch size"
    )
    parser.add_argument(
        "--test-batch-size",
        type=int,
    ...
    parser.add_argument(
        "--epochs", type=int, default=100, metavar="E", help="number of epochs to train"
    )
    parser.add_argument(
        "--max-num-step",
        type=int,
    ...
    parser.add_argument(
        "--momentum", type=float, default=0.5, metavar="M", help="SGD momentum"
    )
    parser.add_argument(
        "--eval-freq",
        type=int,
...
if __name__ == "__main__":
    ...
    if rank == 0:
        ...
        print(
            "This is the server, the world size is {}, cur step: {}".format(
                param_server.world_size, param_server.cur_step
            )
        )
        param_server.start()
        ...
        print(
            "This is the worker: {} in all {} workers".format(
                worker.rank, worker.world_size - 1
            )
        )
